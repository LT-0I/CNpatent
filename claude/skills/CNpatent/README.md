...
- **多 Agent 并行架构** — Planner 生成大纲 → 5 个 Writer Agent 并行写作 → Reviewer Agent 自动审查，大纲确认后全程无需人工干预
...
Phase 0  输入确认 → 场景迁移 → 结构化大纲生成
         ...
         用户确认大纲（唯一确认点）
           ↓
Phase 1  自动任务拆分 → 5 个 Writer Agent 并行生成
           ↓
...
【说明书】
  ...
  技术领域（仅1句）
  背景技术（1-4段）
  发明内容（目的句 → 引入句 → 问题-方案交叉叙事 → 从属特征 → 技术效果）
  附图说明（2段法律声明 + 逐图描述）
  具体实施方式（5段法律声明 + 实施例概述 + 详细步骤 + 技术效果 + 总结 + 结尾）
...
| Writer-B | 发明内容 | ~2000字 |
| Writer-C | 具体实施方式 · 前半 | ~3000字 |
| Writer-D | 具体实施方式 · 后半 | ~3000字 |
| Writer-E | 附图说明 + 摘要 | ~500字 |
