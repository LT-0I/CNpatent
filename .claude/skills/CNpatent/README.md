...
- **三层去 AI 痕迹** — 写作预防 + 正则自动替换 + Reviewer 终审 + `humanizer` / `CNpatent-humanizer` skill 润色
...
> **设计原则**：审查报告、Writer 原始草稿、humanizer 前/后对比等过程产物**不落盘**。这些"修改痕迹"由 Reviewer 在聊天窗口的简要总结呈现；如需精确 diff，由用户在 `outputs/[专利名称]/` 做 `git init` 自行管理。
