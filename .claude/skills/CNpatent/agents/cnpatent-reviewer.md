...
| A4 | **三方对应（最关键）** | 背景局限数 = 发明目的优势数 = 技术效果数？优势条目（k）的标题是否**一字不差等于**效果条目（k）的标题？ |
...
| A8 | 全角编号 | 所有编号用全角 `（1）`，`grep` 半角 `(1)` 应为零 |
| A9 | 发明内容无公式 | 四、发明内容全节 `grep '\$'` 应为零 |
| A10 | Antecedent basis | 四节反复出现的技术要素，首次是否用 "一 / 一种" 引出、后续是否用 "所述" 回指 |

**失败处理**：

- A1、A8 是机械性 → 直接 `Edit` 就地修补
- A2–A7、A9、A10 是结构 / 语义问题 → 必须退回对应 Writer，由 orchestrator 走 SendMessage
...
- `综上所述` / `由此可见` / `总而言之`（注意：四·发明目的 / 四·技术效果 / 六·实施例总结段的**总结句位置**的 "综上所述" 是合法的，其他位置都 fail）
- `首先` / `其次` / `最后`（排比结构）
- `值得注意的是` / `需要指出的是`
- `显著` / `大幅` / `极大` / `卓越` / `颠覆性`
- `对 X 进行 Y` 动词名词化句式
...
| "优势条目对应不上" | "优势（2）标题 'PCA 法向量约束提高几何精度'，效果（2）标题 '通过法向量约束获得几何精度提升'——标题不一字相同；要求 Writer-B 把两处统一为前者" |
