...
3. **文件存在 → 读入并做 schema 校验**：
   ...
   - 四·发明目的要点（含编号优势条目）
   - 四·技术解决方案要点（含编号步骤）
   - 四·技术效果要点（含编号效果条目，与优势条目核心内容相同但措辞角度不同）
   - 五、预计附图清单（4-7 张）
   ...
   **三方对应硬约束**：背景局限 N = 优势条目 N = 效果条目 N，且（k）优势与（k）效果描述同一项改进。优势标题用效果状态描写（"Y较强"），效果标题用动作完成体（"提升了Y"），两者核心内容相同但措辞角度不同（详见 writing-rules.md 的"优势/效果条目标题命名规则"）。
