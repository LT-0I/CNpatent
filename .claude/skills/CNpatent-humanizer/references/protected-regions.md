...
特征：紧跟在"五、附图及附图的简单说明"标题之后，必为前 2 段。

第 1 段固定文本（首句以"为了更清楚地说明"开头）：
```
为了更清楚地说明本发明实施例或现有技术中的技术方案，下面将对实施例中所需要使用的
附图作简单的介绍，显而易见地，下面描述中的附图仅仅是本发明的一些实施例，对于本领域
普通技术人员来讲，在不付出创造性劳动的前提下，还可以根据这些附图获得其他的附图。
```

第 2 段固定文本（首句以"构成本申请的一部分的附图"开头）：
```
构成本申请的一部分的附图用来提供对本申请的进一步理解，本申请的示意性实施例及其说明
用于解释本申请，并不构成对本申请的不当限定。在附图中：
```

**识别**：段首匹配 `^为了更清楚地说明本发明实施例` 或 `^构成本申请的一部分的附图`

### 1.2 具体实施方式前的 5 段法律声明

特征：紧跟在"六、具体实施方式"标题之后，必为前 5 段。

段首关键词（用于识别）：
1. `^现详细说明本发明的多种示例性实施方式`
2. `^应理解本发明中所述的术语仅仅是为描述特别的实施方式`
3. `^在不背离本发明的范围或精神的情况下`
4. `^关于本文中所使用的[「『]包含`
5. `^需要说明的是，在不冲突的情况下`
...
**识别**：段首匹配 `^以上所述，仅为本申请较佳的具体实施方式`

**对法律声明段的处理**：完全不修改。即使段中含有禁用词（例如"本文中所使用
的"包含"本文"），也保留原样——这些是法定固定文本，禁用词检测必须排除它们。

`final_deai_cleanup()` 函数中已经对"本文"做了 lookbehind 排除
（`(?<!本文中所使用的.{0,10})本文`），本 skill 处理时也必须遵循同样原则。
